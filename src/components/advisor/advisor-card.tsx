...
export function AdvisorCard() {
  ...
  const themeQuest = useMemo(() => getQuests(data).find(q => q.themeBonus), [data])
      ...
      <div className="flex items-start gap-3 border-b border-border px-4 py-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className={cn(
            ...
            themeQuest?.done ? 'bg-xp/15 text-xp' : 'bg-surface-2 text-muted',
          )}>
            {theme.emoji} Bu hafta: {theme.title}
            {themeQuest && (
              <span className="tabular-nums">· {themeQuest.done ? '✓' : `${themeQuest.progress}/${themeQuest.target}`}</span>
            )}
          </p>
        </div>
